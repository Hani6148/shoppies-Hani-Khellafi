...
function MovieCardGroup() {
    ...
    const nominatedTitles = nominationList.map(person => person.title)
    ...
    const newList = movies.map(movie => {
        const checkInclud = nominatedTitles.includes(movie.Title)
        if (checkInclud) {
    ...
    return (
        <div class="row movie-container">
            {   newList.map((movie, i) => {
                ...
                return (<MovieCard nominated={movie.nominated} key={shortid.generate()} movieImg={poster ? movie.Poster : defaultPoster} movieTitle={movie.Title} movieYear={movie.Year} />)
            })
            }
        </div>
    )
